...
spotifyController.get(
  ...
  (async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      let user = await userRepository.findOne({ where: { spotifyId: spotifyProfile.id } })
      if (user) {
...
spotifyController.get(
  ...
  (async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({ error: 'Erreur récupération profil (auto refresh)' })
      return
    }
  }) as RequestHandler,
)
...
spotifyController.get(
  ...
  (async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({ error: 'Erreur récupération top artistes' })
      return
    }
  }) as RequestHandler,
)
...
spotifyController.get(
  ...
  (async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({ error: 'Erreur récupération top musiques' })
      return
    }
  }) as RequestHandler,
)
...
spotifyController.get(
  ...
  (async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({ error: 'Erreur récupération playlists' })
      return
    }
  }) as RequestHandler,
)
...
spotifyController.get(
  ...
  (async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      res.status(500).json({ error: 'Erreur recherche Spotify' })
      return
    }
  }) as RequestHandler,
)
    ...
    try {
      ...
      for (const play of response.data.items) {
        await userHistoryRepository.save({
          ...
          artistName: play.track.artists.map((a: any) => a.name).join(', '),
...
spotifyController.get(
  ...
  (async (_req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (users.length < 3) {
        res.status(400).json({ error: 'Il faut au moins 3 utilisateurs pour comparer' })
        return
      }

      res.json({ users: users.map((u) => u.displayName || u.email) })
      return
    } catch {
      res.status(500).json({ error: 'Erreur comparaison utilisateurs' })
      return
    }
  }) as RequestHandler,
)
...
spotifyController.post(
  ...
  (async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      res.status(500).json({ error: 'Erreur rafraîchissement token Spotify' })
      return
    }
  }) as RequestHandler,
)
